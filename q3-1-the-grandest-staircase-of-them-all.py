...
def solution(n):
    # Your code here
    base_cases = [0,1,2] 
    memo_table = [[0 for rows in range(n)] for cols in range(n + 1)]
    
    for base_case in base_cases:
        for column in range(base_case,n):
            memo_table[base_case][column] = 1
    
    for i in range(3, n+1):
        for j in range(2, n):
            if i >= j:
                memo_table[i][j] = memo_table[i][j - 1] + memo_table[i - j][j - 1]
            
            else:
                memo_table[i][j] = memo_table[i][j - 1]
    
    total_combinations = memo_table[n][n-1]
